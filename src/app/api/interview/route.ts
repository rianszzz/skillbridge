...
export async function POST(request: Request) {
  try {
    ...
    const { assessmentId, answer } = body && typeof body === "object" && !Array.isArray(body) ? body : {};
    ...
    const answerCount = await countUserAnswers(session.id);
    if (answerCount >= 5) throw new PublicError("Sesi lima jawaban sudah selesai.", 400, "interview_completed");
    ...
    const answers = session.messages.filter(({ role: r }) => r === "user").map(({ content }) => content);
    ...
    // Complete if 5 answers reached
    const newAnswerCount = answers.length;
    if (newAnswerCount >= 5) await completeInterview(session.id);

    securityLog("interview.completed", { userHash: hashLogValue(user.id), answerCount: newAnswerCount });
    return Response.json({ message, interviewId: session.id, answerCount: newAnswerCount }, privateResponse());
  } catch (error) {
