...
export default function AssessmentForm() {
  ...
  function changeField(next: Field) { setField(next); setFile(undefined); setDescription(""); setUrl(""); setError(""); }
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (field === "informatics") {
        response = await fetch("/api/assess", { method: "POST", headers: await authHeaders(), body: JSON.stringify({ role: roles[field], sourceUrl: url, consent }) });
      } else {
        ...
        response = await fetch("/api/assess", { method: "POST", headers: await authHeaders(false), body: form });
      }
  ...
  return <div className="form-shell"><form className="panel" onSubmit={submit} aria-busy={loading}>{error && <div className="alert" role="alert">{error}</div>}
    ...
    {field === "informatics" ? <div className="field"><label htmlFor="url">URL repositori GitHub publik</label><input id="url" type="url" required placeholder="https://github.com/pemilik/repositori" value={url} onChange={(event) => setUrl(event.target.value)} aria-describedby="evidence-hint"/><p className="hint" id="evidence-hint">{guidance[field].hint}</p></div> : <>
      <div className="field"><label htmlFor="file">{field === "design" ? "Gambar hasil karya" : "PDF laporan kampanye"}</label><input id="file" type="file" required accept={field === "design" ? "image/png,image/jpeg" : "application/pdf"} onChange={(event) => setFile(event.target.files?.[0])} aria-describedby="evidence-hint"/><p className="hint" id="evidence-hint">{guidance[field].hint} Maksimal 4 MB{field === "marketing" ? " dan 15 halaman" : ""}.</p>{file && <p className="chip">{file.name} · {(file.size / 1024).toFixed(0)} KB</p>}</div>
      {field === "design" && <div className="field"><label htmlFor="description">Deskripsi proyek dan proses</label><textarea id="description" required minLength={80} maxLength={5000} rows={7} value={description} onChange={(event) => setDescription(event.target.value)} aria-describedby="description-hint"/><p className="hint" id="description-hint">Minimal 80 karakter. Sertakan brief/masalah, target audiens, keputusan visual, dan minimal dua tahap atau iterasi.</p></div>}
    </>}
