...
export async function saveAssessment(userId: string, result: AssessmentResult, evidenceText: string, stored?: StoredEvidence) {
  ...
  const rubric = await db.from("rubrics").upsert({ career_field: field, target_role: result.role, version: result.rubric_version, criteria: rubrics[result.role], status: "published" }, { onConflict: "career_field,target_role,version" }).select("id").single();
  if (rubric.error) throw rubric.error;
  const evidenceType = stored?.type ?? "github";
  const scores = result.criteria.map((item) => ({ criterion_id: item.criterion_id, criterion_weight: rubrics[result.role].find(({ id }) => id === item.criterion_id)!.weight, evidence_sufficiency: item.evidence_sufficiency, score: item.score, confidence: item.confidence, reason: item.reason, evidence_refs: item.evidence_refs, details: item.details ?? {} }));
...
export async function readAssessments(userId: string, id?: string) {
  ...
  const scores = ids.length ? await db.from("criterion_scores").select("assessment_id,criterion_id,evidence_sufficiency,score,confidence,reason,evidence_refs,details").in("assessment_id", ids) : { data: [], error: null };
